...
export class PCAResults {
    ...
    varianceExplained(copy = true) {
        var output = this.results.variance_explained();
        if (copy) {
            return output.slice();
        } else {
            return output;
 ...
 * @param {?(Uint8WasmArray|Array|TypedArray)} features Array specifying which features should be retained (e.g., HVGs).
 ...
 * @param {number} numberOfPCs Number of top principal components to compute.
 * @param {boolean} scale Whether to scale each feature to unit variance.
 *
